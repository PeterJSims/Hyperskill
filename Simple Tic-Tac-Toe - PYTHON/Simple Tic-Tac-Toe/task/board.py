class Board:
    ...
    def update_cell(self, row: int, col: int) -> None:
        """Turn coordinate (offset by 1) into the current turn's symbol. Afterwards, make a call to switch the symbol."""
    ...
    def row_win_char(self) -> str:
        """Check if each row only contains a single X or O character,thus a winning row. Iterate through all rows in case an
    ...
    def col_win_char(self) -> str:
        """Check if each column only contains a single X or O character, thus a winning column. Iterate through all columns in case an
        impossible game state exists."""
        winning_char = ''
        for i in range(3):
            if len(set([self.board[0][i], self.board[1][i], self.board[2][i]])) == 1 and self.board[0][i] in ('X', 'O'):
                winning_char += self.board[0][i]
        return winning_char

    def diagonal_win_char(self) -> str:
        """Check if each diagonal only contains a single X or O character, thus a winning diagonal. Check both diagonals in case an
        impossible state exists."""
        winning_char = ''
        if len(set([self.board[0][0], self.board[1][1], self.board[2][2]])) == 1 and self.board[0][0] in ('X', 'O'):
            winning_char += self.board[0][0]
        if len(set([self.board[0][2], self.board[1][1], self.board[2][0]])) == 1 and self.board[0][2] in ('X', 'O'):
